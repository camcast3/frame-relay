...
| Screenshot request stays pending or is rejected | The session is no longer active, a collector is offline, or the shared screenshot token does not match | Request it from the active session page while the stream is still live, and confirm the same `FRAME_RELAY_SCREENSHOT_TOKEN` on the hub, host, and collector-capable client |
| Hub works on its own machine but not another device | Firewall source range, routed VLAN policy, or Wi-Fi guest/client isolation | Run `Test-NetConnection <hub-ip> -Port 8080` on the client; allow private LAN/VLAN ranges in the host firewall and permit client VLAN → hub TCP 8080 on the router |
