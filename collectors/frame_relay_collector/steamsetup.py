...
def install_paths(
    *,
    ...
    if system == "Windows":
        ...
        data_root = root / "FrameRelay"
        return InstallPaths(
            system=system,
            data_root=data_root,
            config_path=config_path,
            state_root=data_root / "logs",
            launcher_path=data_root / "bin" / "frame-relay-steam-launch.cmd",
        )
    data_root = (
        Path(env["XDG_DATA_HOME"]) / "frame-relay"
        if env.get("XDG_DATA_HOME")
        else home_path / ".local" / "share" / "frame-relay"
    )
    state_root = (
        Path(env["XDG_STATE_HOME"]) / "frame-relay"
        if env.get("XDG_STATE_HOME")
        else home_path / ".local" / "state" / "frame-relay"
    )
    ...
    return InstallPaths(
        ...
        launcher_path=bin_root / "frame-relay-steam-launch",
    )
...
def legacy_config_path(
    *,
    ...
    if system == "Windows":
        root = Path(env.get("LOCALAPPDATA") or home_path / "AppData" / "Local")
        return root / "ApolloStreamingLab" / "steam-launch.json"
    root = Path(env.get("XDG_CONFIG_HOME") or home_path / ".config")
    return root / "apollo-streaming-lab" / "steam-launch.json"


def legacy_launcher_path(paths: InstallPaths) -> Path:
    if paths.system == "Windows":
        return (
            paths.data_root.parent
            / "ApolloStreamingLab"
            / "bin"
            / "asl-steam-launch.cmd"
        )
    return paths.launcher_path.with_name("asl-steam-launch")
...
def migrate_legacy_profile(
    ...
    *,
    ...
    if paths.config_path.exists():
        return False
    legacy = legacy_config_path(system=system, env=env, home=home)
    if not legacy.is_file():
        return False
    paths.config_path.parent.mkdir(parents=True, exist_ok=True)
    _copy_file_atomic(legacy, paths.config_path)
    print(f"migrated legacy Steam profile: {legacy} -> {paths.config_path}")
    return True
...
def install_payload(paths: InstallPaths, *, repo_root: Path | None = None) -> None:
    ...
    package_source = root / "collectors" / "frame_relay_collector"
    artwork_source = root / "assets" / "steam" / "frame-relay"
    ...
    launcher_source = (
        root / "collectors" / "windows" / "frame-relay-steam-launch.cmd"
        if paths.system == "Windows"
        else root / "collectors" / "linux" / "frame-relay-steam-launch.sh"
    )
    if not launcher_source.is_file():
        raise SetupError(f"Steam launcher not found: {launcher_source}")
    legacy_launcher_source = (
        root / "collectors" / "windows" / "asl-steam-launch.cmd"
        if paths.system == "Windows"
        else root / "collectors" / "linux" / "asl-steam-launch.sh"
    )
    if not legacy_launcher_source.is_file():
        raise SetupError(f"compatibility launcher not found: {legacy_launcher_source}")

    _copy_directory_atomic(package_source, paths.lib_root / "frame_relay_collector")
    _copy_directory_atomic(artwork_source, paths.artwork_root)
    _copy_file_atomic(launcher_source, paths.launcher_path)
    _copy_file_atomic(legacy_launcher_source, legacy_launcher_path(paths))
    if paths.system != "Windows":
        paths.launcher_path.chmod(paths.launcher_path.stat().st_mode | 0o111)
        legacy_launcher_path(paths).chmod(
            legacy_launcher_path(paths).stat().st_mode | 0o111
        )
...
def choose_hub_url(
    ...
    *,
    ...
    if hub_url:
        ...
        value = input_func("Frame Relay hub URL: ").strip()
    else:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="frame-relay-steam-setup",
        description="Install the Frame Relay Steam wrapper and shared client artwork",
    )
...
def run(
    ...
    *,
    ...
    migrate_legacy_profile(paths, system=system, env=env, home=home)
